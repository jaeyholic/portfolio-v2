...
function activate(context) {
    ...
    return entryPointHandler.runApp(APP_NAME, appVersion, errorHelper_1.ErrorHelper.getInternalError(internalErrorCode_1.InternalErrorCode.ExtensionActivationFailed), reporter, function activateRunApp() {
        ...
        context.subscriptions.push(experimentService_1.ExperimentService.create());
...
function onFolderAdded(context, folder) {
    ...
    return projectVersionHelper_1.ProjectVersionHelper.getReactNativeVersions(projectRootPath)
        .then(versions => {
        outputChannelLogger.debug(`React Native version: ${versions.reactNativeVersion}`);
        let promises = [];
        if (projectVersionHelper_1.ProjectVersionHelper.isVersionError(versions.reactNativeVersion)) {
            ...
            telemetryHelper_1.TelemetryHelper.sendErrorEvent("AddProjectReactNativeVersionIsEmpty", errorHelper_1.ErrorHelper.getInternalError(internalErrorCode_1.InternalErrorCode.CouldNotFindProjectVersion), versions.reactNativeVersion, false);
        }
