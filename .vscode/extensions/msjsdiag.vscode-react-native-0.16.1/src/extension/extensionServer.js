...
const experimentService_1 = require("./experimentService/experimentService");
...
class ExtensionServer {
    constructor(projectRootPath, reactNativePackager) {
        ...
        this.experimentService = experimentService_1.ExperimentService.create();
    }
    /**
     * Starts the server.
     */
    ...
    launch(request) {
        ...
        return new Promise((resolve, reject) => {
            ...
            projectVersionHelper_1.ProjectVersionHelper.getReactNativePackageVersionsFromNodeModules(mobilePlatformOptions.projectRoot, true)
                .then(versions => {
                ...
                if (request.arguments.platform === "windows") {
                    if (projectVersionHelper_1.ProjectVersionHelper.isVersionError(versions.reactNativeWindowsVersion)) {
                        throw errorHelper_1.ErrorHelper.getInternalError(internalErrorCode_1.InternalErrorCode.ReactNativeWindowsIsNotInstalled);
                    }
                ...
                if (error && error.errorCode) {
                    if (error.errorCode === internalErrorCode_1.InternalErrorCode.ReactNativePackageIsNotInstalled) {
                    ...
                    else if (error.errorCode === internalErrorCode_1.InternalErrorCode.ReactNativeWindowsIsNotInstalled) {
                        telemetryHelper_1.TelemetryHelper.sendErrorEvent("ReactNativeWindowsPackageIsNotInstalled", errorHelper_1.ErrorHelper.getInternalError(internalErrorCode_1.InternalErrorCode.ReactNativeWindowsIsNotInstalled));
                    }
                }
                this.logger.error(error);
                reject(error);
