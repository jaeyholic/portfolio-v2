...
class Packager {
    ...
    getPackagerArgs(rnVersion, resetCache = false) {
        let args = ["--port", this.getPort().toString()];
        if (resetCache) {
    ...
    start(resetCache = false) {
        ...
        return this.isRunning()
            ...
            return projectVersionHelper_1.ProjectVersionHelper.getReactNativeVersions(this.projectPath)
                .then((versions) => {
                ...
                if (this.runOptions && (this.runOptions.env || this.runOptions.envFile)) {
                    env = generalMobilePlatform_1.GeneralMobilePlatform.getEnvArgument(env, this.runOptions.env, this.runOptions.envFile);
                }
                else {
                    const rootEnv = path.join(this.getProjectPath(), ".env");
                    env = generalMobilePlatform_1.GeneralMobilePlatform.getEnvArgument(env, null, rootEnv);
                }
                ...
                if (this.runOptions && this.runOptions.platform === "exponent") {
                    ...
                    args.push("--sourceExts", managedExtensions);
                }
    ...
    restart(port) {
        if (this.getPort() && this.getPort() !== port) {
            return Q.reject(errorHelper_1.ErrorHelper.getInternalError(internalErrorCode_1.InternalErrorCode.PackagerRunningInDifferentPort, port, this.getPort()));
        }
        return this.isRunning()
            .then(running => {
            if (running) {
    ...
    awaitStart(retryCount = 60, delay = 3000) {
